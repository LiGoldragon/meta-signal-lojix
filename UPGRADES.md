...
the `OpenCodeTesting` node capability, which crosses this contract inside
`ActualizedDeploySubmission`. This changes the encoded wire shape, so peers
must repin to this major release. The contract witness restores a definition
containing that Gold capability from fresh peer bytes.
