...
const MODULE_PATH: &[&str] = &["meta_signal_lojix", "lib"];
const SIGNAL_MODULE_PATH: &[&str] = &["signal_lojix", "lib"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        let ordinary_metadata = CargoEthosSourceMetadata::new("signal-lojix");
        ...
        assert_eq!(
            ordinary_source,
            signal_lojix::LOJIX_INTERFACE_SOURCE,
        ...
        .expect("assemble authority-approved owner Lojix Interface transaction");
        let rust = rust_logos();
        let type_paths = LojixRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project owner Lojix Interface from the verified transaction")
...
fn imported_signal_seats() -> Vec<&'static signal_lojix::bootstrap_manifest::DeclarationSeat> {
    let wanted = IMPORTED_SIGNAL_TYPES
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
    let seats = signal_lojix::bootstrap_manifest::DECLARATION_SEATS
        .iter()
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("owner Lojix bootstrap catalog is complete")
}
